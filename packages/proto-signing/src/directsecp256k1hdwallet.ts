import { encodeSecp256k1Signature, makeCosmoshubPath, rawSecp256k1PubkeyToRawAddress } from "@cosmjs-evm/amino";
import {
  Bip39,
  EnglishMnemonic,
  HdPath,
  ...
  stringToPath,
  Keccak256
} from "@cosmjs-evm/crypto";
import {  fromBase64, fromUtf8, Bech32, toBase64, toBech32, toUtf8, fromHex, toHex, toAscii} from "@cosmjs-evm/encoding";
import { assert, isNonNullObject } from "@cosmjs-evm/utils";
  ...
  public async signDirect(signerAddress: string, signDoc: SignDoc, urlType?: string): Promise<DirectSignResponse> {
    ...
    switch (urlType) {
      ...
      default: {
        ...
        const signature = await Secp256k1.createSignature(hashedMessage, privkey);
        const signatureBytes = new Uint8Array([...signature.r(32), ...signature.s(32)]);
        const stdSignature = encodeSecp256k1Signature(pubkey, signatureBytes,"/cosmos.crypto.secp256k1.PubKey");
