...
export async function main(originalArgs: readonly string[]): Promise<void> {
  ...
  const visiblePackages = (await installedPackages()).filter(
    (name) => name.startsWith("@cosmjs-evm/") || name === "cross-fetch",
  );
  ...
  if (args.selftest) {
    ...
    init += `
      ...
      import {
        ...
        toHex,
        toUtf8,
      } from "@cosmjs-evm/encoding";
      import { sha512, Bip39, Random } from "@cosmjs-evm/crypto";
      import {
        coins,
        encodeAminoPubkey,
        encodeBech32Pubkey,
        ...
        Secp256k1Wallet,
        StdFee,
      } from "@cosmjs-evm/amino";
      import { Decimal } from "@cosmjs-evm/math";
      import { assert, arrayContentEquals, sleep } from "@cosmjs-evm/utils";
