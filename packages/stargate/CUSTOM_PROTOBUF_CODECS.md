...
- You have installed `@cosmjs-evm/proto-signing`, `@cosmjs-evm/stargate` and
  `@cosmjs-evm/tendermint-rpc` as dependencies. In general these dependencies should
  all have the same version, and this document is accurate as of version 0.26.
  ```
  "dependencies": {
    "@cosmjs-evm/proto-signing": "^0.26.4",
    "@cosmjs-evm/stargate": "^0.26.4",
    "@cosmjs-evm/tendermint-rpc": "^0.26.4",
    // ...
  }
  ```
...
script we used for the `@cosmjs-evm/stargate` package
...
`@cosmjs-evm/proto-signing` for you to use, which maps type URLs to codec objects.
For example:

```ts
import { DirectSecp256k1HdWallet, Registry } from "@cosmjs-evm/proto-signing";
import {
  defaultRegistryTypes as defaultStargateTypes,
  SigningStargateClient,
} from "@cosmjs-evm/stargate";
...
```ts
import { createProtobufRpcClient, QueryClient } from "@cosmjs-evm/stargate";
import { Tendermint34Client } from "@cosmjs-evm/tendermint-rpc";
...
Additionally, we provide a system for extending `@cosmjs-evm/stargate`’s
